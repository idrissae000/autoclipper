...
def build_and_train(
    ...
    *,
    ...
    source_videos = sorted(source_videos_dir.rglob("*.mp4")) + sorted(source_videos_dir.rglob("*.mkv"))

    if not source_videos:
        raise ValueError(f"No source videos found in {source_videos_dir}")
...
def main():
    ...
    ap.add_argument("--sources", required=True, type=Path, help="Folder with raw source videos for negatives")
